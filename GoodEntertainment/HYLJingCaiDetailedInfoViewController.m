...
@interface HYLJingCaiDetailedInfoViewController ()<UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UMSocialUIDelegate, UIScrollViewDelegate>
{
...
- (void)createVideoDecriptionView
{
    ...
    //
    _shareButton = [self createCommonButtonWithImage:[UIImage imageNamed:@"jingcaishare"] title:@"分享" x:0 tag:100];
    [headerView addSubview:_shareButton];
        
    _collectButton = [self createCommonButtonWithImage:[UIImage imageNamed:@"myCollectionIcon"] title:@"收藏" x:0 tag:101];
    [headerView addSubview:_collectButton];
        
    _videoDecripCommentButton = [self createCommonButtonWithImage:[UIImage imageNamed:@"jingcaicomment"] title:@"评论" x:0 tag:102];
...
- (void)threeButtonsAction:(UIButton *)sender
{
    switch (sender.tag) {
        ...
        case 100:
        {
            ...
            [UMSocialData defaultData].extConfig.wechatSessionData.title  = self.jingCaiTitle;
            [UMSocialData defaultData].extConfig.wechatTimelineData.title = self.jingCaiTitle;
...
- (void)uploadCommentToServer
{
    ...
    [dictionary setValue:timestamp forKey:@"time"];
    [dictionary setValue:signature forKey:@"sign"];
    [dictionary setValue:self.jingCaiTitle forKey:@"title"];
        ...
        if ([responseDic[@"status"]  isEqual: @1]) {
            ...
            if (dataArray.count > 0) {
                ...
                for (NSDictionary *dic in dataArray) {
                    ...
                    model.created_at = dic[@"updated_at"];
...
{
    _videoDescriptionButton.selected = YES;
...
- (void)changeCommentButtonState
{
    ...
    _videoDescriptionButton.selected = NO;
